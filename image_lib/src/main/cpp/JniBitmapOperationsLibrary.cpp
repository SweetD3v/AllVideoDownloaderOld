...
{
//store
JNIEXPORT jobject JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniStoreBitmapData(
        JNIEnv *env, jobject obj, jobject bitmap);
//get
JNIEXPORT jobject JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniGetBitmapFromStoredBitmapData(
        JNIEnv *env, jobject obj, jobject handle);
//free
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniFreeBitmapData(
        JNIEnv *env, jobject obj, jobject handle);

JNIEXPORT void JNICALL
Java_com_demo_bitmapops_JniBitmapHolder_jniDrawBitmapOnCanvas(
        JNIEnv *env, jobject, jobject handle, jobject canvas, jobject paint);

//rotate 90 degrees CCW
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniRotateBitmapCcw90(
        JNIEnv *env, jobject obj, jobject handle);
//rotate 90 degrees CW
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniRotateBitmapCw90(
        JNIEnv *env, jobject obj, jobject handle);
//rotate 180 degrees
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniRotateBitmap180(
        JNIEnv *env, jobject obj, jobject handle);
//crop
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniCropBitmap(
        JNIEnv *env, jobject obj, jobject handle, uint32_t left,
        uint32_t top, uint32_t right, uint32_t bottom);
//scale using nearest neighbor
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniScaleNNBitmap(
        JNIEnv *env, jobject obj, jobject handle, uint32_t newWidth,
        uint32_t newHeight);

//scale using Bilinear Interpolation
JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniScaleBIBitmap(
        JNIEnv *env, jobject obj, jobject handle, uint32_t newWidth,
        uint32_t newHeight);

JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniFlipBitmapHorizontal(
        JNIEnv *env, jobject obj, jobject handle);

JNIEXPORT void JNICALL Java_com_demo_bitmapops_JniBitmapHolder_jniFlipBitmapVertical(
        JNIEnv *env, jobject obj, jobject handle);
}
...
Java_com_demo_bitmapops_JniBitmapHolder_jniDrawBitmapOnCanvas(
    ...
    jmethodID drawBitmapFunction = env->GetMethodID(canvasCls, "drawBitmap",
                                                    "(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V");

    float x = 0;
    float y = 0;
    float width = jniBitmap->_bitmapInfo.width;
    float height = jniBitmap->_bitmapInfo.height;
    env->CallVoidMethod(canvas, drawBitmapFunction, newBitmap, x, y, width, height, paint);
}
...
Java_com_demo_bitmapops_CartoonUtils_getSketchBitmap(JNIEnv *env, jobject,
                                                     jint width, jint height, jobject orgBitmap) {
    AndroidBitmapInfo info;
    int ret;
    if ((ret = AndroidBitmap_getInfo(env, orgBitmap, &info)) < 0) {
        LOGE("AndroidBitmap_getInfo() failed ! error=%d", ret);
        return NULL;
    }
    ...
    LOGD("reading bitmap pixels...");
    void *bitmapPixels;
    if ((ret = AndroidBitmap_lockPixels(env, orgBitmap, &bitmapPixels)) < 0) {
        LOGE("AndroidBitmap_lockPixels() failed ! error=%d", ret);
        return NULL;
    }
    uint32_t *src = (uint32_t *) bitmapPixels;
    uint32_t *tempPixels = new uint32_t[info.width * info.height];
    int stride = info.stride;
    int pixelsCount = info.width * info.height;
    memcpy(tempPixels, src, sizeof(uint32_t) * pixelsCount);
    AndroidBitmap_unlockPixels(env, orgBitmap);
    LOGE("pixelsCount: %d", pixelsCount);
    return orgBitmap;
}

extern "C" JNIEXPORT jobject JNICALL
Java_com_demo_bitmapops_CartoonUtils_applyGrayscaleNatively(JNIEnv *env, jobject,
...
/**
 * function convertImageToCartoon
 */

//extern "C" JNIEXPORT jintArray JNICALL
//Java_com_demo_bitmapops_CartoonUtils_convertImageToCartoon(JNIEnv *env, jobject){
//
//    //Read input image
...
//    destroyAllWindows();
//    jintArray rgbaData = env->NewIntArray(10);
//    return rgbaData;
//}

//JNIEXPORT jobject JNICALL getBitmap1
//        (JNIEnv *env, jobject, jobject bitmap, Mat src){
//
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_demo_bitmapops_CartoonUtils_getBitmap(JNIEnv *env, jobject, jobject context,
                                               jobject orgBitmap,
    ...
    for (yy = 0; yy < info.height; yy++) {
        tempPixels = (uint32_t *) src;
        for (xx = 0; xx < info.width; xx++) {

            //extract the RGB values from the pixel
            blue = (int) ((tempPixels[xx] & 0x00FF0000) >> 16);
            green = (int) ((tempPixels[xx] & 0x0000FF00) >> 8);
            red = (int) (tempPixels[xx] & 0x00000FF);
        ...
        bitmapPixels = (char *) tempPixels + info.stride;
    }
    int stride = info.stride;
    int pixelsCount = info.width * info.height;
    memcpy(bitmapPixels, src, sizeof(uint32_t) * pixelsCount);
    AndroidBitmap_unlockPixels(env, orgBitmap);
    LOGE("pixelsCount: %d", pixelsCount);
    return orgBitmap;
}